...
#define MOTOS 600 /*Cajones de estacionamiento del 601-1200*/
...
void usuario(){
    ...
    if(tipoVehiculo == 1){
        if((cAutos = contAutos(false)) == -2) return;
        if(cAutos == AUTOS){
        ...
        if((cAutos = contMotos(false)) == -2) return;
        if(cMotos == MOTOS){
            cout << u8"\n\t Lo sentimos, el estacionamiento se encuentra lleno.";
            cout << u8"\n\t Por favor, dé la vuelta. Gracias por su visita.";
            delete v;
        ...
        if((cAutos = contCamiones(false)) == -2) return;
    ...
    if(tipoVehiculo == 1){ 
        if((contAutos(true, cAutos)) == -2) return;
    }else if(tipoVehiculo == 2){
        if((contMotos(true, cMotos)) == -2) return;
    }else{
        if((contCamiones(true, cCamiones)) == -2) return;
    }
    //Si alguno de los anteriores regresa -2 hubo error en lectura o escritura
    ...
    fstream archivo;
    Espacio aux;
    archivo.open(PATH_ADMIN + "estacionamiento.dat", ios::binary|ios::in|ios::out);
    if(!archivo){
    ...
    if(folio < 0) return;  //Error en el archivo de folios
    ...
    archivo.close();
    //Actualizamos la entrada del folio
    if(!actualizar_folios(folio)) return; //Si hubo un error salimos
